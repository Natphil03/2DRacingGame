...
class LapCounter(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.lapscounter = 0
        self.font_name = pygame.font.match_font('arial')
        self.UICounter = pygame.font.Font(self.font_name, 25)

    def update(self):
        self.stringcounter = str(self.lapscounter)
        self.lapcounterText = self.UICounter.render("Lap Counter: ", True, WHITE)
        self.lapcounterTextRect = self.lapcounterText.get_rect()
        self.lapcounterTextRect.midtop = (230, 5)
        self.lapcounterText2 = self.UICounter.render(self.stringcounter, True, WHITE)
        self.lapcounterTextRect2 = self.lapcounterText2.get_rect()
        self.lapcounterTextRect2.midtop = (300, 5)

        gameDisplay.blit(self.lapcounterText, self.lapcounterTextRect)
        gameDisplay.blit(self.lapcounterText2, self.lapcounterTextRect2)

    def lapUpdate(self):
        self.lapscounter = self.lapscounter + 1
        self.stringcounter = str(self.lapscounter)
        #if self.lapscounter >= 3:
        #self.exit = True
        #winningScreen()
...
class Timer(pygame.sprite.Sprite):#timer object
    ...
    def update(self):
        ...
        self.timerText = self.UItext.render("Race Timer: ",True, WHITE)
...
class CountDown(pygame.sprite.Sprite):#countdown object
    ...
    def update(self):
        ...
        if self.countDown < 1:#if countdown is less than 1
            self.text_rectTwo.midtop = (-100, -100)#move text out of view aka removing it
...
class Game:
    def __init__(self):
        ...
        self.exit = False
        self.finish = finish
        self.finishrect = finish.get_rect()
        print(self.finishrect)




    def run(self, currentKart,ts, GAMESTART, loc = (0,0), loc2 = (0,0)):
        player = car(60, 22.5, currentKart)#creates car object
        GAMEEND = time.time()
        self.Carimage = currentKart
        self.carRect = self.Carimage.get_rect()


        #self.outlineMain2 = pygame.mask.from_surface(MainMap2)
        #self.maskoutline = self.outlineMain2.outline()
        #self.n = 0
        #self.loc = loc
        #for point in self.maskoutline:
        #    self.maskoutline[self.n] = (point[0] + self.loc[0], point[1] + self.loc[1])
        #    self.n += 1
        #pygame.draw.polygon(gameDisplay, (255,255,255), self.maskoutline, 5)

        mask = pygame.mask.from_surface(finish)
        mask_outline = mask.outline()
        mask_surf = pygame.Surface(finish.get_size())
        for pixel in mask_outline:
            mask_surf.set_at(pixel, (255, 255, 255))
        mask_surf.set_colorkey((0, 0, 0))

        #Carmask = pygame.mask.from_surface(currentKart)
        #Carmask_outline = Carmask.outline()
        #Carmask_surf = pygame.Surface(MainMap2.get_size())
        #for pixel in Carmask_outline:
        #    Carmask_surf.set_at(pixel, (255, 255, 255))
        #Carmask_surf.set_colorkey((0, 0, 0))
        ...
        if ts == True:
            ...
            Counter = LapCounter()#creates lapcounter object
        ...
        while not self.exit:
            ...
            for event in pygame.event.get():
                ...
                # if self.carRect.colliderect(self.finishrect):
                #     if self.finishrect == self.carRect:
                #         print("hello here")
                    # Counter.lapUpdate()
                    # print("HERE")
                    # if Counter.lapscounter >= 3:
                    #     winningScreen()
                    #     self.exit = True


                if event.type == pygame.MOUSEBUTTONDOWN and mouse_x >= 315 and mouse_x <= 336 and mouse_y >= 64 and mouse_y <= 106:
                    print("HERE")
                    Counter.lapUpdate()
                    if Counter.lapscounter >= 3:
                        winningScreen()
            ...
            gameDisplay.blit(MainMap2,(10,10))#puts main map onto screen

            #map mask
            gameDisplay.blit(mask_surf, (loc[0] - 1, loc[1]))
            gameDisplay.blit(mask_surf, (loc[0] + 1, loc[1]))
            gameDisplay.blit(mask_surf, (loc[0], loc[1] - 1))
            gameDisplay.blit(mask_surf, (loc[0], loc[1] + 1))
            #car mask
            #gameDisplay.blit(Carmask_surf, (loc2[0] - 1, loc2[1]))
            #gameDisplay.blit(Carmask_surf, (loc2[0] + 1, loc2[1]))
            #gameDisplay.blit(Carmask_surf, (loc2[0], loc2[1] - 1))
            #gameDisplay.blit(Carmask_surf, (loc2[0], loc2[1] + 1))

            gameDisplay.blit(finish,(315,72))
    ...
    while KPstatus:#kart picker eventloop
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and mouse_x >= 354 and mouse_x <=  566 and mouse_y >= 334 and mouse_y <= 406:#checks for button presses within x and y coordinate
                ...
                game.run(currentKart, ts, GAMESTART, loc=(10,10))#creates object of main game class
